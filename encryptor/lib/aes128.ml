...
let string_to_state s =
  ...
  for i = 0 to 15 do
    let col = i mod 4 in
    let row = i / 4 in
    Printf.printf "Processing character: %c (ASCII: %d), row: %d, col: %d\n"
      s.[i]
      (Char.code s.[i])
      row col;
    ...
    Array.iter
      (fun r ->
        Array.iter (fun c -> Printf.printf "%d " c) r;
        Printf.printf "\n")
      state;
    Printf.printf "---------\n"
  done;
...
let print_state label state =
  ...
  Array.iter
    (fun row ->
      Array.iter (fun byte -> Printf.printf "%02X " byte) row;
      Printf.printf "\n")
    state;
  Printf.printf "\n"
...
let encrypt message key =
  let blocks = split_into_blocks message in
...
let aes_decrypt_block block round_keys =
  ...
  Printf.printf "After AddRoundKey (Round 10):\n%s\n" (serialize_state !state);

  (* Final round: InvShiftRows, InvSubBytes *)
  state := inv_shift_rows !state;
  Printf.printf "After InvShiftRows (Round 10):\n%s\n" (serialize_state !state);
  state := inv_sub_bytes !state;
  Printf.printf "After InvSubBytes (Round 10):\n%s\n" (serialize_state !state);

  (* Main rounds: AddRoundKey, InvMixColumns, InvShiftRows, InvSubBytes *)
  for round = 9 downto 1 do
    state := add_round_key !state round_keys.(round);
    Printf.printf "After AddRoundKey (Round %d):\n%s\n" round
      (serialize_state !state);
    state := inv_mix_columns !state;
    Printf.printf "After InvMixColumns (Round %d):\n%s\n" round
      (serialize_state !state);
    state := inv_shift_rows !state;
    Printf.printf "After InvShiftRows (Round %d):\n%s\n" round
      (serialize_state !state);
    state := inv_sub_bytes !state;
    Printf.printf "After InvSubBytes (Round %d):\n%s\n" round
      (serialize_state !state)
  done;

  (* Final AddRoundKey *)
  state := add_round_key !state round_keys.(0);
  Printf.printf "After AddRoundKey (Round 0):\n%s\n" (serialize_state !state);
